...
def summarize(text: str, limit: int = 4) -> list[str]:
    terms = {item["term"]: item["count"] for item in keywords(text, 20)}
    ranked = []
    for index, sentence in enumerate(sentences(text)):
        score = sum(terms.get(word.lower(), 0) for word in re.findall(r"[A-Za-z][A-Za-z0-9-]{3,}", sentence))
        ranked.append((score, -index, sentence))
    best = sorted(ranked, reverse=True)[:limit]
    ordered = sorted(best, key=lambda item: -item[1])
    return [item[2] for item in ordered] or ["Add more source text to generate a stronger summary."]
...
def analyze_text(text: str, title: str = "Uploaded material") -> dict:
    ...
    return {
        ...
        "diagram": {
            ...
            "nodes": [item["term"].title() for item in topic_list[:6]] or ["Summary", "Quiz", "Notes"],
        },
    }
